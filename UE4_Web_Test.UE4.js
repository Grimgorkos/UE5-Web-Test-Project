...
console.log("Emscripten version: F:/Programs/UE_4.23/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.31");
console.log("Emscripten configuration: F:/Programs/UE_4.23/Engine/Intermediate/Build/HTML5/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_UE4_Web_Test', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202302252103, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../UE4_Web_Test/UE4_Web_Test.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('UE4_Web_Test.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('UE4_Web_Test.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('UE4_Web_Test.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('UE4_Web_Test.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('UE4_Web_Test.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
